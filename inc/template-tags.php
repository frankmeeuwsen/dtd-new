...
if ( ! function_exists( 'indieweb_publisher_posted_author' ) ) :
	...
	function indieweb_publisher_posted_author() {
		...
		if (  1 === (int) get_option( 'iw_author_url', 0 ) ) {
			...
			$user_avatar = get_avatar( $post_author_id, 32 );
			if ( empty( $user_url ) ) {
		...
		printf(
			'<span class="byline">
				<a class="author-avatar" href="%1$s">
					%4$s
				</a>
				<a href="%1$s" title="%2$s" class="p-author h-card" rel="author">%3$s</a>
			</span>',
			esc_url( $user_url ),
			esc_attr( sprintf( __( 'Bekijk %s author page', 'indieweb-publisher' ), $post_author_nice_name ) ),
			esc_html( $post_author_nice_name ),
			esc_html( $user_avatar)
		);
	}
endif;
